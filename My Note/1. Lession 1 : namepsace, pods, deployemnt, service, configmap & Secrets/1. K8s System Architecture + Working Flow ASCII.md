...
* **ClusterIP** → internal-only communication (inside the node only )
* **NodePort** → access via `<NodeIP>:<Port>` (node to node )
* **LoadBalancer** → external load-balanced endpoint (public by port forwarding) -> k8s service expose the port
